...
with window("About"):
    add_text("Caelum telegram bot")
    add_text("Maintained with <3 by:")
    add_text("Dank-del (github.com/Dank-del) and Kusito (github.com/Stella-lucem")
    add_text("Enviroment:")
    add_text(f"Bot lib: python-telegram-bot v{pver}.", bullet=True)
    add_text(f"Python version: {python_version()}.", bullet=True)
    add_text("Source:")
    add_text("GitHub: github.com/Stella-lucem/Caelum", bullet=True)
