...
@telethn.on(events.NewMessage(pattern=f"^[!/,]los ?(.*)"))
async def los(event):
...
@telethn.on(events.NewMessage(pattern=f"^[!/,]evo ?(.*)"))
async def evo(event):
    if event.sender_id is None:
        return
...
@telethn.on(events.NewMessage(pattern=f"^[!/,]phh ?(.*)"))
async def phh(event):
    if event.sender_id is None:
        return
...
@telethn.on(events.NewMessage(pattern=f"^[!/,]bl ?(.*)"))
...
@telethn.on(events.NewMessage(pattern=f"^[!/,]magisk ?(.*)"))
