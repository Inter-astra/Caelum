...
def about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
	    ...
	    and sender_id is not OWNER_ID
        ):
            message.reply_text(
                "Erm... yeah, I only trust sudo users or owner to set my bio."
            )
            return
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [777000, 1087968824] and sender_id is not OWNER_ID and sender_id not in SUDO_USERS:
            message.reply_text("You are not authorised")
            return

        if user_id == bot.id and sender_id is OWNER_ID:
            message.reply_text("Erm... yeah, I only trust my owner to set my bio.")
            return
