...
def blacklist_mode(update: Update, context: CallbackContext):
    ...
    if args:
        if args[0].lower() in ["off", "nothing", "no"]:
            ...
            return
        if conn:
            text = "Blacklist sticker mode has changed, action is `{}` now at *{}*!".format(
                settypeblacklist, chat_name
            )
        else:
            text = "Blacklisted sticker mode has changed, action is `{}` now!".format(
                settypeblacklist
            )
        ...
        if to_match.set_name.lower() == trigger.lower():
            try:
                if getmode == 0:
                    ...
                    bot.sendMessage(
                        chat.id,
                        "{} muted for {} because using '{}' which is one of the blacklist stickers".format(
                            mention_markdown(user.id, user.first_name), value, trigger
                        ),
                        parse_mode="markdown",
                    )
