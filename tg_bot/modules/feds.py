...
def new_fed(update, context):
    ...
    if len(fednam) >= 2:
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Report in @{SCHAT} if the problem persists."
            )
            return
...
INFO_FED_HANDLER = CommandHandler(["fedinfo", "finfo"], fed_info, pass_args=True)
...
FED_BROADCAST_HANDLER = CommandHandler(["fbroadcast", "fcast"], fed_broadcast, pass_args=True)
...
FEDSTAT_USER = DisableAbleCommandHandler(
    ["fedstat", "fbanstat", "fstat"], fed_stat_user, pass_args=True
)
